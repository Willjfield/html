...
var viewMaterial;
var highlightGeo,highlightNROMat,highlightMilMat,highlightUnMat,highlightMesh;
...
function init() {
       ...
       //Background
        ...
        var geometryObs= new THREE.SphereGeometry( 0.5, 48, 48 );
...
function animate(time) {
    ...
    myViewPosition=new THREE.Vector3();
    myViewPosition.copy(myThreePosition);
    var changeThisForOffset = .5;
    longRotation = ((xpl.now+timeOffset+sumT+changeThisForOffset)%(xpl.planets[2].dayLength/23.93331))*(2*Math.PI)//*(Math.PI*1.5)//+.2//Why+.2? //.0069//0.78539816339+.6//+0.04363323127;
...
function createSats(){
                ...
                for (var i = 0; i < tle_data.length; i ++ ) {
                                                ...
                                                vertex.x = tle_data[i].position_eci.x*0.0156;
                                                vertex.y = tle_data[i].position_eci.z*0.0156;
                                                vertex.z = tle_data[i].position_eci.y*-0.0156;
    ...
    switch(tourStage){
        ...
        case 1:
            ...
            if(camera.position.distanceTo(new THREE.Vector3())<1500){
                controls.dollyOut(1+(0.01*multiplier));
                multiplier*=0.999;
            }        
        break;
        ...
        case 5:
            if(createDiv){
                ...
                controls.rotateLeft(0.5*(1-viewerNormalizedLong.dot(cameraNormalizedLong)));
                var mappedLatitude = (180-(obs.latitude+90))*xpl.DEG2RAD;
                var distanceFromLatitude = controls.getPolarAngle()-mappedLatitude;
                
                if(Math.abs(distanceFromLatitude)>0.01){
                    controls.rotateUp(0.1*distanceFromLatitude);
                }

                if( controls.target.distanceTo(myViewPosition)>0.1){
                    ...
                    controls.target.x-=diffX*0.1;
                    controls.target.y-=diffY*0.1;
                    controls.target.z-=diffZ*0.1;
                }
